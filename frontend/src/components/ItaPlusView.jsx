...
export default function ItaPlusView({ onBack }) {
  const [step, setStep]             = useState('upload')   // upload | column | review | done
  ...
  const handleFile = useCallback(async (file) => {
    ...
    try {
      ...
      // Pre-seleziona la colonna chiamata "ITA" se esiste
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28 }}>
        <button onClick={onBack} style={{ background: 'none', color: 'var(--text2)', fontSize: 13 }}>
          ← Menu
        </button>
        <div style={{ width: 1, height: 16, background: 'var(--border)' }} />
        <span style={{ fontSize: 15, fontWeight: 600 }}>ITA+</span>
        {filename && (
          <span style={{ fontSize: 13, color: 'var(--text2)', fontFamily: 'var(--mono)' }}>{filename}</span>
        )}
      </div>
        ...
        <div style={{ maxWidth: 480 }}>
          <div style={{
            ...
            <p style={{ fontWeight: 500, marginBottom: 8 }}>
              Quale colonna contiene le frasi da riformulare?
            </p>
            <p style={{ color: 'var(--text2)', fontSize: 13, marginBottom: 16 }}>
              Di solito è la colonna "ITA"
            </p>
            <select
              value={colIndex}
          ...
          <button onClick={handleProcess} disabled={loading} style={{
            ...
            {loading ? '✨ Riformulazione in corso… (può richiedere 30-60 secondi)' : '✨ Riformula con AI'}
          </button>
        </div>
      )}
        ...
        <>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <p style={{ color: 'var(--text2)', fontSize: 13 }}>
              Clicca sulla colonna <strong style={{ color: 'var(--text)' }}>ITA+</strong> per modificare una riga prima di salvare
            </p>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {rows.map((row, i) => (
                  <ReviewRow key={i} row={row} index={i} onEdit={handleEdit} />
                ))}
              </tbody>
            </table>
          </div>
    ...
    <tr style={{ borderBottom: '1px solid var(--border)' }}>
      ...
      <td style={{ ...td, cursor: 'text' }} onClick={() => !editing && setEditing(true)}>
        {editing ? (
          ...
          <span style={{ color: val ? '#a0f0c0' : 'var(--text2)' }}>
            {val || '—'}
          </span>
        )}
      </td>
