...
import ItaPlusView from './components/ItaPlusView.jsx'

const API = import.meta.env.VITE_API_URL || ''

export default function App() {
  const [mode, setMode]   = useState('menu')   // 'menu' | 'ita' | 'itaplus'
  const [step, setStep]   = useState('upload') // 'upload' | 'criteria' | 'table'
  const [sessionId, setSessionId]     = useState(null)
  const [filename, setFilename]       = useState('')
  const [rows, setRows]               = useState([])
  const [proposedColors, setProposed] = useState([])
  ...
  const handleUpload = useCallback(async (file) => {
    ...
    try {
      ...
      setSessionId(data.session_id); setFilename(data.filename); setStep('criteria')
  ...
  const handleProcess = useCallback(async (criteria) => {
    ...
    try {
      ...
      setProposed(data.proposed_colors)
      setStep('table'); setShowColorModal(true)
    } catch (e) { setError(e.message) }
    finally { setLoading(false) }
  }, [sessionId])
  ...
  const handleColors = useCallback(async (assignments) => {
    ...
    finally { setLoading(false); setShowColorModal(false) }
  }, [sessionId])
  ...
  const handleRework = useCallback(async (sentences) => {
    ...
    return (await res.json()).reworked
  }, [])
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      a.download = 'soprattitoli.xlsx'; a.click()
  ...
  const handleReset = () => {
    setMode('menu'); setStep('upload'); setSessionId(null)
    setFilename(''); setRows([]); setProposed([]); setError('')
  }

  // ── Percorso ITA+ ────────────────────────────────────────────────────────────
  if (mode === 'itaplus') {
    return <ItaPlusView onBack={handleReset} />
  }

  // ── Menu iniziale ────────────────────────────────────────────────────────────
  if (mode === 'menu') {
    return <MenuScreen onChoose={(m) => setMode(m)} />
  }
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <TopBar
        step={step} filename={filename} stats={stats}
        onColorClick={() => setShowColorModal(true)}
        onExport={handleExport} onReset={handleReset} loading={loading}
      />

      {error && (
        <div style={{
          margin: '12px 24px 0', padding: '10px 16px',
          background: '#2d1a1a', border: '1px solid #7a2020',
          borderRadius: 8, color: '#ff8080', fontSize: 14
        }}>
          ⚠ {error}
          <button onClick={() => setError('')} style={{ float: 'right', background: 'none', color: '#ff8080', fontSize: 16 }}>×</button>
        </div>
      )}

      <main style={{ flex: 1, padding: step === 'table' ? 0 : '40px 24px' }}>
        {step === 'upload' && <UploadStep onUpload={handleUpload} loading={loading} />}
        {step === 'criteria' && (
          <CriteriaModal filename={filename} onConfirm={handleProcess}
            onBack={() => setStep('upload')} loading={loading} />
        )}
        {step === 'table' && (
          <TableView rows={rows} onUpdateRow={handleUpdateRow} onRework={handleRework} />
        )}
      </main>

      {showColorModal && (
        <ColorModal proposed={proposedColors}
          onAccept={handleColors} onSkip={() => setShowColorModal(false)}
